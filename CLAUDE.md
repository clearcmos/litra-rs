...
- **The full QML lint only runs here, not in CI.** Run the script with no arguments on this workstation, where Plasma 6 is installed and types resolve; that is the real check. A GitHub runner has the Qt tools but none of the QtQuick or Plasma QML modules, so `PlasmoidItem` and every Plasma property are unknown names there and no type checking is possible at all. CI therefore runs `--parse-only`, which uses `qmlformat` (no warning categories, so nothing to drift) purely to confirm `main.qml` parses. That is worth gating, since a syntax error there breaks the widget outright, but do not read a green CI as "the QML is lint-clean". Run the full check locally before trusting that. `litra.mjs` is deliberately not passed to `qmlformat`: 6.4 cannot detect a `.mjs` file's type and 6.11 can, and it needs no cover here anyway since `node --test` imports it and fails on a syntax error before any assertion runs.
